...
function carrega_listas(){
  ...
  document.getElementById('abel3').style.display = 'none';
...
function analisedefala() {
  ...
  for (var i = 0; i < 7; i++) {
    ...
    for (var j = 0; j < lista.length; j++) {
      ...
      if (resultado.search(aux) >= 0) {


        indices[i].letra = letrascorresp[j];

        indices[i].valor = 1 + resultado.indexOf(aux);

        res = aux.length;

        subst = sub.substr(1, res);

        resultado = resultado.replace(aux, subst);
  ...
  for (var i = 0; i < 7; i++) {
    ...
    if (indices[i].valor > 0) { placafalada += indices[i].letra; }
